...
void max_percolate_up(Queue* queue, int index)
{
    ...
    if (index > 0 && queue->nodes[index]->priority >= queue->nodes[parent]->priority)
    {
...
void max_percolate_down(Queue* queue, int index)
{
    ...
    int right = 2 * index + 2;
    int max = index;
    if (left < queue->capacity && queue->nodes[left]->priority >= queue->nodes[max]->priority)
    {
        if (queue->nodes[left]->priority > queue->nodes[max]->priority){
            max = left;
        }
    }
    if (right < queue->capacity && queue->nodes[right]->priority >= queue->nodes[max]->priority)
    {
